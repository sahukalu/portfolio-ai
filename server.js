...
function localProfileKB(prompt) {
  ...
  if (p.includes("who are you") || p.includes("who am i")) {
    return "I am **SK Pinkun**, Kalu's virtual AI assistant 🤖. I'm here to help you learn about his skills, experience, education, and how to get in touch. What would you like to know?";
  }
  if (p.includes("kalu") || p.includes("name")) {
    return "Kalu Ch Sahu is a **DevOps Engineer** & multi-cloud specialist. He is currently a 6th-semester B.Tech student at Gayatri College, passionate about cloud technology and network security.";
  }
  if (p.includes("skills") || p.includes("skill") || p.includes("tech stack")) {
    return "Skills: **Azure AKS, Docker, Terraform, CI/CD, Server Administration, Network Security, Aruba Networking, Cisco Networking, GitHub Actions, Python, Linux**.";
  }
  if (p.includes("education") || p.includes("college")) {
    return "B.Tech in Computer Science at Gayatri College of Engineering (6th Semester). Focused on cloud and AI research.";
  }
  if (p.includes("certi") || p.includes("az-") || p.includes("certification")) {
    return "Certifications: **AZ-400 (DevOps Expert), AZ-204, AZ-900, AZ-700, AZ-104**, and **Commvault Cloud Administrator 2026**.";
  }
  ...
  if (p.includes("experience") || p.includes("work")) {
    return "Kalu is a DevOps Engineer working with multi-cloud and network security architectures. He has hands-on experience with Kubernetes orchestration, server security, CAST AI optimization, Aruba networking, Cisco networking, and Azure AI Search.";
  }
  if (p.includes("docker")) {
    return "Docker is a platform that packages applications and their dependencies into lightweight containers for consistent deployment.";
  }
  if (p.includes("terraform")) {
    return "Terraform is an Infrastructure-as-Code tool that lets you provision and manage cloud resources using declarative configuration files.";
  }
  if (p.includes("kubernetes") || p.includes("k8s") || p.includes("aks")) {
    return "Kubernetes (K8s) is an open-source container orchestration platform. Azure Kubernetes Service (AKS) is Microsoft's managed Kubernetes offering.";
  }
